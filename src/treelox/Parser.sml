...
struct
  ...
  and classDeclaration sts =
    let
      fun parseMethods (acc, sts) =
        if check ([T.RightBrace, T.Eof], sts) then
          (rev acc, sts)
        else
      ...
      val (superclass, sts) =
        ...
        | NONE => (NONE, sts)
      val (_, sts) = consume (T.LeftBrace, "Expect '{' before class body.", sts)
      val (methods, sts) = parseMethods ([], sts)
      val (_, sts) = consume (T.RightBrace, "Expect '}' after class body.", sts)
      val class =
  ...
  and function (kind, sts) =
    let
      ...
      val (_, sts) = consume
        (T.LeftParen, "Expect '(' after " ^ kind ^ " name.", sts)
      val (params, sts) =
        if check ([T.RightParen], sts) then ([], sts) else parseParams ([], sts)
      val (_, sts) = consume (T.RightParen, "Expect ')' after parameters.", sts)
      val (_, sts) = consume
        (T.LeftBrace, "Expect '{' before " ^ kind ^ " body.", sts)
  ...
  and forStatement sts =
    let
      val (_, sts) = consume (T.LeftParen, "Expect '(' after 'for'.", sts)
      val (st, sts') = advance sts
      val (initializer, sts) =
        case #token st of
      ...
      val (increment, sts) =
        if check ([T.RightParen], sts) then (NONE, sts)
        else let val (expr, sts) = expression sts in (SOME expr, sts) end
      val (_, sts) = consume
        (T.RightParen, "Expect ')' after for clauses.", sts)
  ...
  and ifStatement sts =
    let
      val (_, sts) = consume (T.LeftParen, "Expect '(' after 'if'.", sts)
      val (cond, sts) = expression sts
      val (_, sts) = consume
        (T.RightParen, "Expect ')' after if condition.", sts)
  ...
  and whileStatement sts =
    let
      val (_, sts) = consume (T.LeftParen, "Expect '(' after 'while'.", sts)
      val (cond, sts) = expression sts
      val (_, sts) = consume (T.RightParen, "Expect ')' after condition.", sts)
      val (body, sts) = statement sts
    in
  ...
  and block (decs, sts) =
    if check ([T.RightBrace, T.Eof], sts) then
      let val (_, sts) = consume (T.RightBrace, "Expect '}' after block.", sts)
      in (rev decs, sts)
      end
    else
  ...
  and equality sts =
    binary ([T.BangEqual, T.EqualEqual], comparison) sts

  and comparison sts =
    binary ([T.Greater, T.GreaterEqual, T.Less, T.LessEqual], term) sts
  ...
  and call sts =
    let
      fun tail (expr, sts) =
        ...
        in
          case #token st of
            T.LeftParen => tail (finishCall (expr, sts'))
          | T.Dot =>
              let
  ...
  and finishCall (callee, sts) =
    let
      ...
      val (arguments, sts) =
        if check ([T.RightParen], sts) then ([], sts) else parseArgs ([], sts)
      val (paren, sts) =
        consume (T.RightParen, "Expect ')' after arguments.", sts)
    in
    ...
    in
      ...
      | T.LeftParen =>
          let
            val (expr, sts') = expression sts'
            val (_, sts') = consume
              (T.RightParen, "Expect ')' after expression.", sts')
          in
